...
enum inv_iim423xx_clock_source{
	//INV_IIM423XX_PLL,
	INV_IIM423XX_RC_OSC,
	INV_IIM423XX_WU_OSC,
	INV_IIM423XX_CLOCK_SOURCE_MAX
};
...
typedef struct clk_calib{
	...
	uint8_t on_going;
	float    coef[INV_IIM423XX_CLOCK_SOURCE_MAX];   /**< Calibration coefficient */
	uint64_t last_timestamp_sent[INV_IIM423XX_SENSOR_MAX]; /**< last timestamp sent when using the extended timestamp API */
	uint32_t last_fifo_timestamp[INV_IIM423XX_SENSOR_MAX]; /**< last timestamp read from FIFO */
 ...
 *  @param[in] states     placeholder to inv_iim423xx_t states
 ...
 */
int inv_helper_extend_timestamp_from_fifo(struct inv_iim423xx * s, struct clk_calib *clk_cal, 
...
/** @brief Reset calibration data linked to acc sensors. This is typically used when 
 *  enabling a sensor.
 *
 *  Note: This function should be called when enabling Accel
 *
 *  @param[in] states     placeholder to inv_iim423xx_t states
 *  @param[in] clk_calib  placeholder to clk_calib_t states
 *  @param[in] sensor     sensor to be restarted (INV_IIM423XX_SENSOR_ACCEL or INV_IIM423XX_SENSOR_ACCEL)
 */
void clock_calibration_reset_sensors_stats(struct inv_iim423xx * s, struct clk_calib *clk_cal, enum inv_iim423xx_sensor );
 ...
 * This function finds the smallest ODR of running sensors among ACC. Then it restarts
 * the time deviation computation based on this ODR.
 *
 *  Note: This function should be called when enabling Accel or when changing ODR
 *
 *  @param[in] states     placeholder to inv_iim423xx_t states
 *  @param[in] clk_calib  placeholder to clk_calib_t states
 */
